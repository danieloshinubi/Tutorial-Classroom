...
export function buildEmail(input: BuildEmailInput): { html: string; text: string } {
  ...
  const logoCell = school.logoUrl
    ...
    : `<div style="width:38px;height:38px;border-radius:10px;background:${pal.brand};text-align:center;line-height:38px;font-family:Georgia,'Times New Roman',serif;color:#ffffff;font-size:16px;font-weight:700;">${initial}</div>`;
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" class="sv-wrap" style="width:560px;max-width:560px;background:#ffffff;border-radius:14px;overflow:hidden;border:1px solid #e8e8ee;font-family:-apple-system,'Segoe UI',Helvetica,Arial,sans-serif;">
...
<tr><td class="sv-pad" style="padding:28px 36px 0;">
  <table role="presentation" cellpadding="0" cellspacing="0"><tr>
    <td>${logoCell}</td>
    <td style="padding-left:11px;font-size:14.5px;font-weight:700;color:#191623;">${schoolName}</td>
  </tr></table>
</td></tr>

<tr><td class="sv-pad" style="padding:26px 36px 0;">
  <span style="display:inline-block;background:${badgeBg};color:${badgeColor};font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;padding:4px 10px;border-radius:999px;">${escapeHtml(badge)}</span>
</td></tr>

<tr><td class="sv-pad" style="padding:14px 36px 0;">
  <h1 class="sv-heading" style="margin:0;font-size:21px;line-height:1.3;color:#191623;font-weight:750;">${escapeHtml(heading)}</h1>
</td></tr>

<tr><td class="sv-pad" style="padding:14px 36px 0;font-size:14.5px;line-height:1.65;color:#464054;word-break:break-word;">
  ${bodyHtml}
</td></tr>
...
${ctaLabel && ctaUrl ? `<tr><td class="sv-pad" style="padding:26px 36px 4px;">
  <table role="presentation" cellpadding="0" cellspacing="0" class="sv-cta"><tr>
    <td style="border-radius:10px;background:${pal.brand};">
      <a href="${escapeHtml(ctaUrl)}" style="display:inline-block;padding:12px 26px;font-size:14px;font-weight:650;color:#ffffff;text-decoration:none;border-radius:10px;">${escapeHtml(ctaLabel)}</a>
    </td>
  </tr></table>
</td></tr>` : ""}

<tr><td class="sv-pad" style="padding:30px 36px 28px;">
  <div style="border-top:1px solid #efeef3;padding-top:16px;font-size:12px;line-height:1.7;color:#9a94a8;">
...
<div style="font-size:11px;color:#b3aec2;margin-top:16px;">Sent via Schoolivio on behalf of ${schoolName}</div>
</td></tr>
</table>
</body>
</html>`;
