-- =============================================================================
-- Scope decide_application() and promote_applicant_to_student() to a school
--
-- Both functions load the application by id alone, then authorize the
-- caller via can_finalise_admission(app.school_id)/can_do_admissions(app.school_id)
-- /has_role_in(app.school_id, ...) — correctly checking THAT application's
-- actual school, but with no way to also require it be the tenant currently
-- open. An owner/admin/principal/admissions staffer at more than one school
-- could decide or enrol an application belonging to a DIFFERENT one of
-- their schools while believing they were acting on the current tenant, if
-- a stale application id ever reached the client.
--
-- Both signatures gain a required school parameter and an early check that
-- the application's actual school_id matches it, before anything else runs.
-- =============================================================================

drop function if exists classroom.decide_application(uuid, classroom.application_status, text, timestamptz, text);
...
begin
  ...
  if app.school_id <> current_school then
    raise exception 'This application does not belong to the current school';
  end if;
...
grant execute on function classroom.decide_application(uuid, classroom.application_status, uuid, text, timestamptz, text) to authenticated;

drop function if exists classroom.promote_applicant_to_student(uuid, uuid, uuid);
...
begin
  ...
  if app.school_id <> target_school then
    raise exception 'This application belongs to a different school';
  end if;
...
grant execute on function classroom.promote_applicant_to_student(uuid, uuid, uuid, uuid) to authenticated;
