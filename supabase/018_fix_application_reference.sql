-- =============================================================================
-- Fix: public application submission was failing outright
--
--   FOR UPDATE is not allowed with aggregate functions
--
-- The reference number was picked with `select max(seq) + 1 ... for update`, to
-- stop two simultaneous submissions being handed the same number. Postgres
-- does not allow FOR UPDATE alongside an aggregate, so the statement was
-- invalid and every submission failed.
--
-- An advisory lock does the same job properly: it serialises submissions for
-- one school and session without needing to lock rows, and is released when
